...
def get_scores(targets, preds, n):
    ...
    """
    scores = {}
    scores['accuracy'] = np.sum(targets==preds)/len(targets)
...
class MadryDatasetNLP(DataSet):
    def __init__(self, data_path, **kwargs):
        ...
        super(MadryDatasetNLP, self).__init__(ds_name,
                                           data_path, **ds_kwargs)
...
def adv_scores(adv_model, dataset, scores, score, constraint='inf', eps=0.04, iterations=3, 
    ...
    dataset.transform = transform
    loader = DataLoader(dataset, batch_size=batch_size, sampler=tools.make_weighted_sampler(dataset), 
...
def adv_scores_helper(adv_model, loader, attack_kwargs, scores, score):
    ...
    for inp, target in loader:
        if attack_kwargs['constraint'] in ['2', 'inf']:
            _, im_adv = adv_model(inp.cuda(), target.cuda(), make_adv=True, **attack_kwargs)
            adv_model.model.eval()
            output = adv_model.model(im_adv.cuda())
        else: 
